...
CONSTRAINT [FK_TIPO_PRODUTO] FOREIGN KEY ([COD_TIPO_PRODUTO]) REFERENCES
...
CONSTRAINT [FK_COD_PEDIDO_VENDA] FOREIGN KEY ([COD_PEDIDO_VENDA]) REFERENCES
PEDIDO_VENDA(COD_PEDIDO_VENDA),
CONSTRAINT [FK_PRODUTO] FOREIGN KEY ([COD_PRODUTO]) REFERENCES
PRODUTO(COD_PRODUTO)
);
CREATE UNIQUE INDEX [IPK_ITEM_PEDIDO_VENDA] ON [ITEM_PEDIDO_VENDA]
([COD_ITEM_PEDIDO_VENDA]);
...
CONSTRAINT [FK_COD_PEDIDO_COMPRA] FOREIGN KEY ([COD_PEDIDO_COMPRA]) REFERENCES
PEDIDO_COMPRA(COD_PEDIDO_COMPRA),
CONSTRAINT [FK_PRODUTO] FOREIGN KEY ([COD_PRODUTO]) REFERENCES
PRODUTO(COD_PRODUTO)
);
CREATE UNIQUE INDEX [IPK_ITEM_PEDIDO_COMPRA] ON [ITEM_PEDIDO_COMPRA]
...
INSERT INTO PRODUTO (COD_PRODUTO, NOME, VLR_PRODUTO_COMPRA, VLR_PRODUTO_VENDA,
COD_TIPO_PRODUTO)
...
INSERT INTO COTACAO (COD_COTACAO, VLR_COTACAO, UNIDADE_MEDIDA, COD_PRODUTO,
COD_FORNECEDOR)
...
INSERT INTO ITEM_PEDIDO_COMPRA (COD_ITEM_PEDIDO_COMPRA, QTD_ITEM_PEDIDO_COMPRA,
COD_PRODUTO, COD_PEDIDO_COMPRA)
VALUES (1, 10, 2, 1);
-- Inserindo um registro na tabela ITEM_PEDIDO_COMPRA
INSERT INTO ITEM_PEDIDO_COMPRA (COD_ITEM_PEDIDO_COMPRA, QTD_ITEM_PEDIDO_COMPRA,
COD_PRODUTO, COD_PEDIDO_COMPRA)
...
INSERT INTO ITEM_PEDIDO_VENDA (COD_ITEM_PEDIDO_VENDA, QTD_ITEM_PEDIDO_VENDA,
COD_PEDIDO_VENDA, COD_PRODUTO)
VALUES (1, 10, 2, 1);
-- Inserindo um registro na tabela ITEM_PEDIDO_VENDA
INSERT INTO ITEM_PEDIDO_VENDA (COD_ITEM_PEDIDO_VENDA, QTD_ITEM_PEDIDO_VENDA,
COD_PEDIDO_VENDA, COD_PRODUTO)
...
INSERT INTO PEDIDO_VENDA (COD_PEDIDO_VENDA, DATA_PEDIDO, VLR_PEDIDO_VENDA,
COD_VENDEDOR, COD_CLIENTE, COD_SITUACAO_PEDIDO, COD_FORMA_PAGAMENTO)
...
UPDATE FORMA_PAGAMENTO SET DES_FORMA_PAGAMENTO = 'CRÉDITO A VISTA' WHERE
COD_FORMA_PAGAMENTO = 1;
UPDATE SITUACAO_PEDIDO SET STATUS_PEDIDO= 'DEVOLVIDO' WHERE COD_SITUACAO_PEDIDO =
3;
UPDATE ITEM_PEDIDO_COMPRA SET QTD_ITEM_PEDIDO_COMPRA= 300 WHERE
COD_ITEM_PEDIDO_COMPRA = 1;
UPDATE ITEM_PEDIDO_VENDA SET QTD_ITEM_PEDIDO_VENDA = 400 WHERE
COD_ITEM_PEDIDO_VENDA = 1;
...
ORDER BY pc.VLR_PEDIDO_COMPRA DESC
LIMIT 1;

-- 2: Pedido de venda mais recente
SELECT pv.COD_PEDIDO_VENDA, pv.DATA_PEDIDO, c.NOME AS NOME_CLIENTE
FROM PEDIDO_VENDA pv
INNER JOIN CLIENTE c ON c.COD_CLIENTE = pv.COD_CLIENTE
ORDER BY pv.DATA_PEDIDO DESC
LIMIT 1;

-- 3: Pedidos com situação "CANCELADO"
SELECT pv.COD_PEDIDO_VENDA, sp.STATUS_PEDIDO, sp.STATUS_PAGAMENTO
FROM PEDIDO_VENDA pv
INNER JOIN SITUACAO_PEDIDO sp ON sp.COD_SITUACAO_PEDIDO = pv.COD_SITUACAO_PEDIDO
WHERE sp.STATUS_PEDIDO = 'CANCELADO';

-- 4: Fornecedores com mais cotações
SELECT f.COD_FORNECEDOR, f.NOME AS NOME_FORNECEDOR, COUNT(c.COD_COTACAO) AS TOTAL_COTACOES
FROM FORNECEDOR f
INNER JOIN COTACAO c ON c.COD_FORNECEDOR = f.COD_FORNECEDOR
GROUP BY f.COD_FORNECEDOR, f.NOME
ORDER BY TOTAL_COTACOES DESC
LIMIT 1;

-- 5: Produtos com cotações mais caras
SELECT c.COD_COTACAO, c.VLR_COTACAO, p.NOME AS NOME_PRODUTO
FROM COTACAO c
INNER JOIN PRODUTO p ON p.COD_PRODUTO = c.COD_PRODUTO
ORDER BY c.VLR_COTACAO DESC
LIMIT 1;

-- PT EXTRA Todos os clientes e seus pedidos (incluindo clientes sem pedidos)
SELECT c.NOME AS NOME_CLIENTE, pv.COD_PEDIDO_VENDA, pv.DATA_PEDIDO
FROM CLIENTE c
LEFT JOIN PEDIDO_VENDA pv ON c.COD_CLIENTE = pv.COD_CLIENTE;

-- Todos os fornecedores e suas cotações (incluindo fornecedores sem cotações)
SELECT f.NOME AS NOME_FORNECEDOR, c.COD_COTACAO, c.VLR_COTACAO
FROM FORNECEDOR f
RIGHT JOIN COTACAO c ON f.COD_FORNECEDOR = c.COD_FORNECEDOR;
